...
(define (apply-generic op . args)
  ...
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags)))
      (if proc
          (helper op (apply proc (map contents args)))
          (if (= (length args) 2)
              (let ((a1 (car args))
                    (a2 (cadr args)))
                (let ((a1->a2 (raise-into a1 a2))
                      (a2->a1 (raise-into a2 a1)))
                  (cond (a1->a2
                         (helper op (apply-generic op a1->a2 a2)))
                        (a2->a1
                         (helper op (apply-generic op a1 a2->a1)))
...
(define (install-integer-package)
  ...
  (put 'sqroot '(integer) (lambda (x) (make-real (sqrt x))))
  (put 'arctan '(integer integer) (lambda (y x) (make-real (atan y x))))
  (put 'make 'integer
...
(define (install-rational-package)
  ...
  (put 'project '(rational)
       (lambda (x) (make-integer (/ (numer x) (denom x)))))
  (put 'sine '(rational) (lambda (x) (make-real (sin (/ (numer x) (denom x))))))
  (put 'cosine '(rational) (lambda (x) (make-real (cos (/ (numer x) (denom x))))))
  (put 'sqroot '(rational) (lambda (x) (make-real (sqrt (/ (numer x) (denom x))))))
  (put 'arctan '(rational rational)
       (lambda (y x) (make-real (atan (/ (numer y) (denom y))
                                      (/ (numer x) (denom x))))))
  (put 'make 'rational
  ...
  (put 'project '(real)
       (lambda (x)
         (let ((exact-x (inexact->exact x)))
           (let ((n (numerator exact-x))
                 (d (denominator exact-x)))
             (make-rational n d)))))
  'done)
